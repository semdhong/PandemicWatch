...
using Pegasus.Command.Server.Authorization;
using Pegasus.Command.Server.Helpers;
using Pegasus.Command.Server.Managers;
using Pegasus.Command.Server.Middleware;
...
using Pegasus.Command.Storage;
using Pegasus.Command.Storage.Mapping;
...
namespace Pegasus.Command.Server
{
    public class Startup
    {
        ...
        {
            var authAuthority = Configuration["Pegasus.Command:IS4ApplicationUrl"].TrimEnd('/');
            ...
            else
            {
                ...
                if (Convert.ToBoolean(Configuration["Pegasus.Command:UseLocalCertStore"]) == true)
                {
                    var certificateThumbprint = Configuration["Pegasus.Command:CertificateThumbprint"];
            ...
            services.Configure<IdentityOptions>(options =>
            {
                ...
                if (Convert.ToBoolean(Configuration["Pegasus.Command:RequireConfirmedEmail"] ?? "false"))
                {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseMiddleware<APIResponseRequestLoggingMiddleware>(Convert.ToBoolean(Configuration["Pegasus.Command:EnableAPILogging:Enabled"] ?? "true"));
